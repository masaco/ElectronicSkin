...
public class GetArmRotation : MonoBehaviour
{
	...
	public bool gestureReset = false;

	// use for transformer
	public GameObject comparer;
	public float comparerDistance;
	public float comparerAngle;
	public float comparerYrot;
    ...
    void Update ()
    {
		...
		comparerDistance = Vector2.Distance (new Vector2 (comparer.transform.position.x, comparer.transform.position.y), new Vector2 (transform.position.x, transform.position.y));
		Vector3 diff = comparer.transform.position - transform.position;
		Vector2 diffVec2 = new Vector2 (diff.x, diff.y);
		comparerAngle = Vector2.Angle (new Vector2 (0.0f, 1.0f), diffVec2.normalized);
		if( diffVec2.x > 0.0f )
			comparerAngle *= -1;
		comparerYrot = comparer.transform.rotation.eulerAngles.z;
